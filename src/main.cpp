...
#if defined(NDEBUG)
# error "Sasincoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xd32dc5889f7e2c684228892c08e15203bf890bba6134d6e7b21b94edf811eea5");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Sasincoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Sasincoin Signed Message:\n";
...
{
    // Sasincoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 7000000); // Sasincoin: 7m blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Sasincoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Sasincoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Sasincoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Sasincoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x6d918ec985489c173d57991c0801630bb27898e05c8d42055d7e3f44077153f0");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "16-10-2020 - powstaje sasincoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 7 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("048b3ac34cf08204b36356575b5986f917ab27b8f26426f25e95e86b092711fdfd5eff0f5a01739ac29c8d9a8209c536fab257d4534b15258ee14ac603666ea720") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1602846913;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085253183;

        if (fTestNet)
        {
            block.nTime    = 1602846895;
            block.nNonce   = 385285991;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x5893d7f5c5a91cdeef88d152b8f892f759534ca85505b4fe45b0ade3892fa6f8"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc1, 0xb3, 0xdu }; // Sasincoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SasincoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SasincoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SasincoinMiner(CWallet *pwallet)
{
    printf("SasincoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("sasincoin-miner");
    ...
    try { loop {
        ...
        printf("Running SasincoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SasincoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SasincoinMiner, pwallet));
}
